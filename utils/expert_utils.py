...
def ffn_expert_fn(input_size,
  ...
  """
  def my_fn(x):
    layer_sizes = [input_size] + model_ds + [output_size]
    for i in range(1 + len(model_ds)):
...
def local_moe_tpu(inputs,
  ...
  hidden layer of size model_d, followed by an output projection.

  The number of parameters is thus:
    num_experts * (input_size * model_d + model_d * output_size)
  ...
  h = common_layers.batch_dense(
      expert_inputs, model_d, activation=tf.nn.relu, name="x0")
