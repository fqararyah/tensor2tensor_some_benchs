...
""" from tensor2tensor import memory_saving_gradients
...
gradients.__dict__["gradients"] = gradients_speed  """
...
def conv_block_downsample(x,
  ...
  with tf.variable_scope(
    ...
    model_d = int(x.get_shape()[-1])
    res = conv_block(
        x,
        int(1.25 * model_d), [((1, 1), kernel)],
        padding=padding,
        strides=strides,
        name="res_conv")
...
def conv_relu_conv(inputs,
  ...
  with tf.variable_scope(name, "conv_relu_conv", [inputs]):
    ...
    h = tpu_conv1d(
        inputs, d_ff, first_kernel_size, padding=padding, name="conv1")
...
def conv_hidden_relu_memory_efficient(x,
  ...
  Args:
    ...
    d_ff: an integer - size of the hidden layer.
  ...
  with tf.variable_scope(name, default_name="ffn2", values=[x]):
    ...
    if test_vars is not None:
      ...
      f1 = tf.get_variable("f1", [1, io_size, d_ff])
      f2 = tf.get_variable("f2", [1, d_ff, io_size])
...
def deep_discriminator(x,
  ...
  with tf.variable_scope(
    ...
    net = layers().Conv2D(
        filters, d_ff, strides=stride, padding="SAME", name="conv1")(x)
    net = lrelu(net)
    net = layers().Conv2D(
        2 * filters,
        d_ff,
...
def patch_discriminator(x, filters=64, d_ff=5, n=4,
                        name="patch_discrim"):
  """Patch descriminator."""
  with tf.variable_scope(name):
    x_shape = shape_list(x)
