...
def create_session_config(log_device_placement=False,
  ...
  gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=gpu_mem_fraction)#fareed
...
class T2TExperiment(object):
  ...
  def train(self, max_steps=None):
    ...
    with open('/home/nahmad/prof/wrn/params.txt', 'w') as f:
        f.write("num_params: " + str(num_params))
    tf.logging.set_verbosity(tf.logging.INFO)
    #logging_hook = tf.train.LoggingTensorHook({"loss" : 0}, every_n_iter=10)
