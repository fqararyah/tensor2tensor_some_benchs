      ...
      with tf.variable_scope("layer_%d" % layer):
        ...
        with tf.variable_scope("gated_linear_unit"):
          ...
          values = common_layers.layers().Dense(
              hparams.model_d)(hidden_state)
          gates = common_layers.layers().Dense(
              hparams.model_d, activation=tf.nn.sigmoid)(hidden_state)
        ...
        with tf.variable_scope("conv_branches"):
          ...
          mask = tf.tile(
              tf.expand_dims(nonpadding, 2), [1, 1, hparams.model_d])
          hidden_state *= mask

          left_output_dim = int(hparams.model_d * 4)
          ...
          right_output_dim = int(hparams.model_d / 2)
          ...
          hidden_state = tf.pad(
              hidden_state,
              [[0, 0], [0, 0], [0, hparams.model_d - right_output_dim]],
              constant_values=0)
        ...
        with tf.variable_scope("dense_layers"):
          ...
          hidden_state = common_layers.layers().Dense(
              int(hparams.model_d * 4), activation=tf.nn.relu)(hidden_state)
    ...
    for layer in range(num_layers):
      ...
      with tf.variable_scope(layer_name):
        ...
        with tf.variable_scope(_CONV_BRANCHES_NAME):
          ...
          left_output_dim = int(hparams.model_d * 2)
          ...
          if nonpadding is not None:
            # Mask padding from conv layers.
            mask = tf.tile(
                tf.expand_dims(nonpadding, 2), [1, 1, hparams.model_d * 2])
            hidden_state *= mask
        ...
        with tf.variable_scope("dense_layers"):
          ...
          hidden_state = tf.layers.dense(hidden_state, hparams.model_d)
          hidden_state = common_layers.layer_postprocess(
              residual_state, hidden_state, hparams)
...
def add_evolved_transformer_hparams(hparams):
  ...
  hparams.learning_rate_schedule = (
      "constant*linear_warmup*single_cycle_cos_decay*rsqrt_model_d")
  return hparams
