...
class DiscreteBottleneck(object):
  ...
  def __init__(self, hparams):
    ...
    self.hparams.block_dim = int(
        self.hparams.model_d // self.hparams.num_blocks)
  ...
  def slice_hidden(self, x):
    ...
    Args:
        x: Encoder hidden state of shape [-1, model_d].
  ...
  def embed(self, x):
    ...
    h1_shape = shape_x
    h1_shape.append(self.hparams.model_d)
    ...
    h2 = tf.layers.dense(tf.nn.relu(h1), self.hparams.d_ff, name="vch2")
    res = tf.layers.dense(
        tf.nn.relu(h2), self.hparams.model_d, name="vcfin")
    return res
