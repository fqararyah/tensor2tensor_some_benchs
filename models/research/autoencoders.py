...
class AutoencoderBasic(t2t_model.T2TModel):
  ...
  def body(self, features):
    ...
    if (hparams.mode != tf.estimator.ModeKeys.PREDICT
      ...
      self._cur_bottleneck_tensor = b
      res_size = self.hparams.model_d * 2**self.hparams.num_hidden_layers
      res_size = min(res_size, hparams.max_model_d)
    ...
    # Final dense layer.
    res = tf.layers.dense(
        res, self.num_channels * hparams.model_d, name="res_dense")

    output_shape = common_layers.shape_list(res)[:-1] + [
        self.num_channels, self.hparams.model_d
    ]
    ...
    with tf.variable_scope("encoder"):
      ...
      for i in range(hparams.num_hidden_layers):
        with tf.variable_scope("layer_%d" % i):
          ...
          layers.append(x)
          x = self.dropout(x)
          filters = hparams.model_d * 2**(i + 1)
          filters = min(filters, hparams.max_model_d)
...
def autoencoder_basic():
  ...
  hparams.add_hparam("max_model_d", 1024)
...
def autoencoder_residual_discrete():
  ...
  hparams.add_hparam("isemhash_d_ff_multiplier", 2.0)
...
def autoencoder_discrete_pong_range(rhp):
  ...
  rhp.set_discrete("max_model_d", [1024, 2048])
