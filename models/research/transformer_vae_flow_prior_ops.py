...
def decoder(name, latents, hparams, decoder_self_attention_bias, **kwargs):
  ...
  with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    ...
    if hparams.pos_attn:
      ...
      decoder_input = common_attention.add_timing_signal_1d(decoder_input)
    if common_layers.shape_list(latents)[-1] != hparams.model_d:
      decoder_input = gops.dense("lat2hid", latents, hparams.model_d)
    decoder_output = transformer_decoder_layers(
        "block",
    ...
    decoder_output = tf.reshape(
        decoder_output, [batch_size, targets_length, 1, hparams.model_d])
...
def drop_2d(targets, mode, dropout_p):
  ...
  if dropout_p > 0 and mode == tf.estimator.ModeKeys.TRAIN:
    batch_size, targets_length, model_d = common_layers.shape_list(targets)
    mask_prob = tf.random_uniform(
        shape=(batch_size, targets_length), minval=0.0, maxval=1.0)
    mask_prob = tf.tile(mask_prob[..., tf.newaxis], [1, 1, model_d])
    scale = 1 / (1 - dropout_p)
...
def lenpred_mlp(name, logits, model_d, bound):
  with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    logits = tf.layers.dense(logits, model_d)
...
def predict_target_lengths(
  ...
  if hparams.predict_target_length:
    ...
    logits = lenpred_mlp("lenpred", logits, hparams.model_d, bound)
