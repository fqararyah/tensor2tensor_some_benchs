...
class NextFrameEmily(sv2p.NextFrameSv2pLegacy):
  ...
  def stacked_lstm(self, inputs, states, model_d, output_size, nlayers):
    ...
    net = inputs
    net = tfl.dense(
        net, model_d, activation=None, name="af1")
    for i in range(nlayers):
      net, states[i] = common_video.basic_lstm(
          net, states[i], model_d, name="alstm%d"%i)
    net = tfl.dense(
        net, output_size, activation=tf.nn.tanh, name="af2")
    return net, states

  def lstm_gaussian(self, inputs, states, model_d, output_size, nlayers,
                    name):
    ...
    """
    net = inputs
    net = tfl.dense(net, model_d, activation=None, name="%sf1"%name)
    for i in range(nlayers):
      net, states[i] = common_video.basic_lstm(
          net, states[i], model_d, name="%slstm%d"%(name, i))
