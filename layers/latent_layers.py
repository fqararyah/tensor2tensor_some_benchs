...
def compress_self_attention_layer(x, hparams, name=None):
  ...
  with tf.variable_scope(name, default_name="compress_self_attention"):
    ...
    y = common_attention.multihead_attention(
        ...
        None,
        None,
        hparams.attention_key_channels or hparams.model_d,
        hparams.attention_value_channels or hparams.model_d,
        hparams.model_d, hparams.num_heads,
...
def ae_latent_sample_beam(latents_dense_in, inputs, ed, embed, hparams):
  ...
  Args:
    ...
    inputs: Tensor of shape [batch, length_kv, hparams.model_d]. Encodings
...
def residual_block_layer(inputs, hparams):
  ...
  Args:
    inputs: Tensor of shape [batch, height, width, hparams.model_d].
    hparams: HParams.

  Returns:
    Tensor of shape [batch, height, width, hparams.model_d].
  """
  ...
  for i in range(hparams.num_res_layers):
    with tf.variable_scope("res_conv_%d" % i):
      ...
      y = common_layers.conv_block(
          common_layers.layer_norm(x, hparams.model_d, name="lnorm"),
...
def compress_encoder(inputs,
  ...
  Returns:
    Tensor of shape [batch, latent_length, hparams.model_d], where
      latent_length is
  ...
  with tf.variable_scope(name, default_name="compress"):
    ...
    x = tf.layers.dense(x,
                        hparams.num_latents * hparams.model_d,
                        name=name + "_dense")
...
def decompress_decoder(inputs,
  ...
  with tf.variable_scope(name, default_name="decompress"):
    x = inputs
    x = tf.layers.dense(x, hparams.model_d, name=name + "_dense")
    x = residual_block_layer(x, hparams)
    for i in range(hparams.num_compress_steps // 2):
...
def decompress_decoder_1d(x, hparams, name=None):
  ...
  Returns:
    Tensor of shape [batch, length, hparams.model_d].
  """
  x = tf.expand_dims(x, axis=2)
...
def transformer_text_encoder(inputs,
  ...
  Args:
    inputs: Tensor of shape [batch, length, 1, hparams.model_d].
    target_space: int. Used for encoding inputs under a target space id.
    hparams: HParams.
    name: string, variable scope.

  Returns:
    encoder_output: Tensor of shape [batch, length, hparams.model_d].
...
def transformer_image_decoder(targets,
  ...
  Args:
    targets: Tensor of shape [batch, ...], and whose size is batch * height *
      width * hparams.num_channels * hparams.model_d.
    encoder_output: Tensor of shape [batch, length_kv, hparams.model_d].
  ...
  with tf.variable_scope(name, default_name="transformer_dec"):
    ...
    targets = tf.reshape(targets, [batch_size,
                                   hparams.img_len,
                                   hparams.img_len,
                                   hparams.num_channels * hparams.model_d])
...
def transformer_latent_decoder(x,
  ...
  Args:
    x: Tensor of shape [batch, length_q, hparams.model_d]. length_q is the
  ...
  Returns:
    Tensor of shape [batch, length_q, hparams.model_d].
  """
...
def latent_prediction_model(inputs,
  ...
  Args:
    inputs: Tensor of shape [batch, length_kv, hparams.model_d]. Inputs to
    ...
    latents_dense: Tensor of shape [batch, length_q, hparams.model_d].
  ...
  Returns:
    latents_pred: Tensor of shape [batch, length_q, hparams.model_d].
...
def transformer_autoencoder(inputs,
  ...
  Args:
    inputs: Tensor of shape [batch, length, 1, hparams.model_d] or None.
  ...
  Returns:
    decoder_output: Tensor of shape [batch, ..., hparams.model_d] presenting
  ...
  if hparams.mode != tf.estimator.ModeKeys.PREDICT:
    ...
    latents_dense = tf.zeros([batch_size, latent_len, 1, hparams.model_d])
    if cache is None:
    ...
    latents_dense = embed_fn(cache_one_hot, hparams.model_d)
  ...
  latents_dense = tf.reshape(
      latents_dense,
      [-1, hparams.img_len, hparams.img_len, hparams.model_d])
