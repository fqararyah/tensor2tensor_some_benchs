...
class Aligned(t2t_model.T2TModel):
  ...
  def body_sharded(self, sharded_features):
    ...
    extra_loss = 0.0
    ffn_model_ds = [int(s) for s in hparams.ffn_model_ds.split(",")]
      ...
      with tf.variable_scope("%s_%d" % (layer_type, layer_num)):
        ...
        if layer_type == "timing":
          ...
          y = dp(common_attention.multihead_attention, x, None,
                 ...
                 hparams.model_d, hparams.attention_value_channels or
                 hparams.model_d, hparams.model_d, hparams.num_heads,
                 hparams.attention_dropout)
          ...
          y = dp(
              expert_utils.ffn_expert_fn(hparams.model_d, ffn_model_ds,
                                         hparams.model_d),
...
def aligned_base():
  ...
  hparams.add_hparam("ffn_model_ds", "2048")  # Add new ones like this.
