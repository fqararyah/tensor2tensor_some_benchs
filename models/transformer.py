...
class Transformer(t2t_model.T2TModel):
  ...
  def _fast_decode(self,
    ...
    if hparams.pos == "timing":
      ...
      positional_encoding = common_attention.add_timing_signals_from_features(
          tf.zeros([1, decode_length, hparams.model_d]), features,
          hparams.position_features)
    elif hparams.pos == "emb":
      positional_encoding = common_attention.add_positional_embedding(
          tf.zeros([1, decode_length, hparams.model_d]), hparams.max_length,
...
def _init_transformer_cache(cache, hparams, batch_size, attention_init_length,
  ...
  key_channels = hparams.attention_key_channels or hparams.model_d
  value_channels = hparams.attention_value_channels or hparams.model_d
  num_layers = hparams.num_decoder_layers or hparams.num_hidden_layers
  vars_3d_num_heads = (
  ...
  # won't be changed to
  # `[beamsize*batch_size, decode_length, hparams.model_d]` and may cause
    ...
    for layer in range(hparams.num_decoder_layers or hparams.num_hidden_layers):
      ...
      if hparams.memory_type == "neural_memory":
        memory = transformer_memory.TransformerMemory(
            ...
            key_depth=hparams.model_d,
            val_depth=hparams.model_d,
...
def transformer_base_v1():
  ...
  hparams.add_hparam("attention_key_channels", 128)
  hparams.add_hparam("attention_value_channels", 128)
  hparams.add_hparam("ffn_layer", "dense_relu_dense")
  hparams.add_hparam("parameter_attention_key_channels", 128)
  hparams.add_hparam("parameter_attention_value_channels", 128)
...
def transformer_base_v3():
  ...
  hparams.learning_rate_schedule = (
      "constant*linear_warmup*rsqrt_decay*rsqrt_model_d")
...
@registry.register_hparams
def transformer_52_2048_4096():
  """HParams for transformer big model on WMT."""
  hparams = transformer_base_v2()
  hparams.num_encoder_layers = 52
  hparams.num_decoder_layers = 52
  hparams.model_d = 2048
  hparams.d_ff = 4096
  hparams.num_heads = 16
  hparams.activation_dtype = "float32" 
  return hparams
...
@registry.register_hparams
def transformer_48_2048_8192():
  """HParams for transformer big model on WMT."""
  hparams = transformer_base_v2()
  hparams.num_encoder_layers = 48
  hparams.num_decoder_layers = 48
  hparams.model_d = 2048
  hparams.d_ff = 8192
  hparams.num_heads = 8
  hparams.max_length = 32
...
def transformer_tall_pretrain_lm_tpu_adafactor_large():
  ...
  hparams.model_d = 1024
  hparams.num_heads = 16
  hparams.d_ff = 32768  # max fitting in 16G memory is 49152, batch 2
...
@registry.register_hparams
def transformer_pardnn():
  """HParams for transformer base model for single GPU."""
  hparams = transformer_base()
  hparams.learning_rate_schedule = "constant*linear_warmup*rsqrt_decay"
  hparams.learning_rate_constant = 0.1
  hparams.learning_rate_warmup_steps = 16000
  hparams.num_encoder_layers = 16
