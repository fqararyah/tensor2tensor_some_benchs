...
def nas_decoder(decoder_input,
  ...
  Args:
    ...
    hparams: transformer.Transformer hparams that must also contain:
      ...
      + enforce_output_size: Boolean that determines whether or not the decoder
        output must be resized to hparams.model_d. If True, the output will
        be resized if it not equal to hparams.model_d. If False, the output
  ...
  (_, output_depth, _, _) = calculate_branching_model_parameters(
      encoding_depth=hparams.model_d,
      ...
      encoder_depth=hparams.model_d)
  improper_output_size = output_depth != hparams.model_d
