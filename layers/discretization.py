...
def project_hidden(x, projection_tensors, model_d, num_blocks):
  """Project encoder hidden state under num_blocks using projection tensors.

  Args:
    x: Encoder hidden state of shape [batch_size, latent_dim,  model_d].
    projection_tensors: Projection tensors used to project the hidden state.
    model_d: Dimension of the latent space.
  ...
  x = tf.reshape(x, shape=[1, -1, model_d])
  x_tiled = tf.reshape(
      tf.tile(x, multiples=[num_blocks, 1, 1]),
      shape=[num_blocks, -1, model_d])
...
def slice_hidden(x, model_d, num_blocks):
...
def embed(x,
  ...
  Args:
    ...
    model_d: Dimension of the latent state.
    z_size: Number of bits, where discrete codes range from 1 to 2**z_size.
    d_ff: Dimension to project embedding by. Used only if bottleneck_kind
      is semhash.
  ...
  with tf.variable_scope(name, default_name="embed", reuse=tf.AUTO_REUSE):
    if bottleneck_kind == "semhash":
      ...
      h1a = tf.layers.dense(c, d_ff, name="vch1a")
      h1b = tf.layers.dense(1.0 - c, d_ff, name="vch1b")
      h1 = h1a + h1b
      h1 = tf.layers.dense(h1, model_d, name="vch_final_linear")

    elif bottleneck_kind == "gumbel-softmax":
      hot = tf.one_hot(x, 2**z_size)
      h1 = tf.layers.dense(hot, model_d, name="dae_dense")
...
def discrete_bottleneck(inputs,
  ...
  Args:
    ...
    model_d: Dimension of the dense output.
    z_size: Number of bits, where discrete codes range from 1 to 2**z_size.
    d_ff: Filter size in the embedding function.
    mode: tf.estimator.ModeKeys.
  ...
  Returns:
    outputs_dense: Tensor of shape [..., output_dim]. The output dimension is
      model_d if bottleneck_kind is gumbel-softmax, DVQ; d_ff if
  ...
  with tf.variable_scope(
    ...
    if bottleneck_kind == "dense":
      ...
      outputs_dense = tf.layers.dense(
          outputs_discrete, d_ff, name="vch1")
      ...
      if reshape_method == "slice":
        x_reshaped = slice_hidden(
            inputs_3d, model_d=model_d, num_blocks=num_blocks)
      ...
      # Reshape assuming model_d == inputs_shape[:-1].
      ...
      outputs_dense = tf.layers.dense(
          outputs_hot, model_d, name="dae_dense")
      neg_q_entropy = tf.constant(0.0)
      ...
      c = tf.where(
          ...
          y_discrete, y)
      outputs_dense_a = tf.layers.dense(c, d_ff, name="vch1a")
      outputs_dense_b = tf.layers.dense(1.0 - c, d_ff, name="vch1b")
      outputs_dense = outputs_dense_a + outputs_dense_b
      outputs_dense = tf.layers.dense(outputs_dense, model_d,
...
def get_vq_codebook(codebook_size, model_d):
  """Get lookup table for VQ bottleneck."""
  with tf.variable_scope("vq", reuse=tf.AUTO_REUSE):
    means = tf.get_variable(
        name="means",
        shape=[codebook_size, model_d],
...
def vq_body(x,
  ...
  model_d = x_shape[-1]
  means, ema_means, ema_count = get_vq_codebook(codebook_size, model_d)
  x = tf.reshape(x, [-1, model_d])
...
def vq_loss(x,
  ...
  model_d = x_shape[-1]
  means, _, _ = get_vq_codebook(codebook_size, model_d)
  x = tf.reshape(x, [-1, model_d])
...
def gumbel_softmax_nearest_neighbor_dvq(x,
  ...
  if num_flows > 0:
    hparams = iaf_hparams(model_d=512, d_ff=4096)
    ...
    for flow in range(num_flows):
      ...
      # [batch_size, latent_size, model_d].
...
def gumbel_softmax_discrete_bottleneck(x,
  ...
  x_shape = common_layers.shape_list(x)
  model_d = x_shape[-1]
  means, ema_means, ema_count = get_vq_codebook(bottleneck_size, model_d)
  x = tf.reshape(x, [-1, model_d])
...
def tanh_discrete_unbottleneck(x, model_d):
  """Simple un-discretization from tanh."""
  x = tf.layers.dense(x, model_d, name="tanh_discrete_unbottleneck")
  return x
...
def isemhash_unbottleneck(x, model_d, isemhash_d_ff_multiplier=1.0):
  """Improved semantic hashing un-bottleneck."""
  d_ff = int(model_d * isemhash_d_ff_multiplier)
  x = 0.5 * (x - 1.0)  # Move from [-1, 1] to [0, 1].
  with tf.variable_scope("isemhash_unbottleneck"):
    h1a = tf.layers.dense(x, d_ff, name="hidden1a")
    h1b = tf.layers.dense(1.0 - x, d_ff, name="hidden1b")
    h2 = tf.layers.dense(tf.nn.relu(h1a + h1b), d_ff, name="hidden2")
    return tf.layers.dense(tf.nn.relu(h2), model_d, name="final")
...
def parametrized_unbottleneck(x, model_d, hparams):
  """Meta-function calling all the above un-bottlenecks with hparams."""
  if hparams.bottleneck_kind == "tanh_discrete":
    return tanh_discrete_unbottleneck(x, model_d)
  if hparams.bottleneck_kind == "isemhash":
    return isemhash_unbottleneck(x, model_d,
                                 hparams.isemhash_d_ff_multiplier)
  if hparams.bottleneck_kind in ["vq", "em", "gumbel_softmax"]:
    return vq_discrete_unbottleneck(x, model_d)
  raise ValueError(
      "Unsupported hparams.bottleneck_kind %s" % hparams.bottleneck_kind)


def iaf_hparams(model_d=512, d_ff=4096):
  """Create hyperpameters for inverse autoregressive flows.

  Args:
    model_d: Width of attention layers and neural network output layer.
    d_ff: Hidden layer width for neural network.
