...
def decompress_step(source, hparams, first_relu, is_2d, name):
  ...
  with tf.variable_scope(name):
    ...
    thicker = common_layers.conv_block(
        source, hparams.model_d * multiplier, [((1, 1), kernel)],
        first_relu=first_relu, name="decompress_conv")
    if is_2d:
      return tf.depth_to_space(thicker, 2)
    return tf.reshape(thicker, [shape[0], shape[1] * 2, 1, hparams.model_d])
    ...
    if task == "translate":
      ...
      if not hparams.drop_inputs:
        decoder_input += tf.reshape(
            inputs,
            [common_layers.shape_list(targets)[0], 1, 1, hparams.model_d])
...
def ae_transformer_internal(inputs,
  ...
  targets = tf.reshape(targets, [batch_size, -1, 1, hparams.model_d])
...
class TransformerAE(t2t_model.T2TModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self._hparams.bottleneck = functools.partial(
        ...
        model_d=self._hparams.model_d,
        z_size=self._hparams.z_size,
        d_ff=self._hparams.d_ff,
    ...
    if self._hparams.bottleneck_kind in ["dvq", "gumbel-softmax-dvq"]:
      ...
      block_dim = int(self._hparams.model_d // self._hparams.num_blocks)
      ...
      if self._hparams.reshape_method == "project":
        ...
        projection_tensors = tf.get_variable(
            ...
            shape=[
                ...
                self._hparams.model_d, block_dim
            ],
  ...
  def prepare_features_for_infer(self, features):
    ...
    inputs = tf.zeros([beam_batch_size, 1, 1, self._hparams.model_d])
    inputs = inputs if "inputs" in features else None
    if self._hparams.drop_inputs or not self.has_input:
      inputs = None
    targets = tf.zeros([beam_batch_size, 1, 1, self._hparams.model_d])
...
def transformer_ae_small():
  ...
  hparams.model_d = 384
  hparams.d_ff = 2048
  hparams.add_hparam("compress_d_ff", 2048 * 2)
...
def imagetransformer_ae_cifar():
  ...
  # TODO(trandustin): semhash doesn't work if d_ff != model_d. For
