...
def universal_transformer_depthwise_attention(layer_inputs,
  ...
  states_so_far_weights = tf.nn.softmax(
      common_layers.dense(
          states_so_far, (hparams.model_d if hparams.dwa_elements else 1),
          activation=None,
          use_bias=True),
      axis=-1)
...
def _ffn_layer_multi_inputs(inputs_list,
  ...
  assert hparams.model_d == common_layers.shape_list(main_input)[-1]
  ...
  if ffn_layer_type == "dense":
    output = common_layers.dense(
        ffn_inputs,
        hparams.model_d if output_size is None else output_size,
        name=name,
