...
def init_vq_bottleneck(bottleneck_size, model_d):
  """Get lookup table for VQ bottleneck."""
  means = tf.get_variable(
      name="means",
      shape=[bottleneck_size, model_d],
...
def vq_discrete_bottleneck(x, hparams):
  ...
  x = tf.reshape(x, [-1, hparams.model_d])
...
def vq_discrete_unbottleneck(x, hparams):
  ...
  result = tf.reshape(result, x_shape[:-1] + [hparams.model_d])
  return result
...
def decompress_step(source, hparams, first_relu, name):
  ...
  with tf.variable_scope(name):
    ...
    thicker = common_layers.conv_block(
        source,
        hparams.model_d * multiplier, [((1, 1), kernel)],
...
def decode_transformer(encoder_output, encoder_decoder_attention_bias, targets,
  ...
  with tf.variable_scope(name):
    ...
    decoder_output = tf.reshape(
        decoder_output, [decoder_output_shape[0], -1, 1, hparams.model_d])
...
def ae_transformer_internal(inputs, targets, target_space, hparams, cache=None):
  ...
  # Postprocess.
  d = latents_dense
  pos = tf.get_variable("pos", [1, 1000, 1, hparams.model_d])
...
class TransformerNAT(t2t_model.T2TModel):
  ...
  def __init__(self, *args, **kwargs):
    ...
    means, ema_means, ema_count = init_vq_bottleneck(
        2**self._hparams.bottleneck_bits, self._hparams.model_d)
  ...
  def prepare_features_for_infer(self, features):
    ...
    inputs = tf.zeros([batch_size, 1, 1, self._hparams.model_d])
    inputs = inputs if "inputs" in features else None
    targets = tf.zeros([batch_size, 1, 1, self._hparams.model_d])
