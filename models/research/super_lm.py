...
def _super_stack(inputs,
  ...
  moe_model_ds = [int(s) for s in hparams.moe_model_ds.split(",")]
  if hparams.diet_experts:
    ...
    expert_fn = expert_utils.ffn_expert_fn(
        hparams.model_d, moe_model_ds, hparams.model_d)
  ...
  mix_size = int(hparams.mix_fraction * hparams.model_d)
  accumulator = inputs
  x = inputs
  extra_losses = []
      ...
      if layer_type == "a":
        ...
        def _split(t):
          return tuple(tf.split(
              t, [mix_size, hparams.model_d - mix_size], 2))
        ...
        q = mp(tf.layers.dense, x, hparams.model_d, use_bias=False,
               name="q_transform")
        x = mp(
            common_attention.scaled_dot_product_attention_simple,
            q, x, x, attention_bias_3d)
        x = mp(tf.layers.dense, x, hparams.model_d, use_bias=False,
               name="o_transform")
        ...
        x = mp(
            ...
            None,
            attention_bias,  # bias
            hparams.multihead_attention_key_channels or hparams.model_d,
            hparams.multihead_attention_value_channels or hparams.model_d,
            hparams.model_d,
