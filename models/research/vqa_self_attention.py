...
class VqaSelfAttention(vqa_attention.VqaAttentionBaseline):
  ...
  def body(self, features):
    ...
    image_model_d = hp.image_model_d or hp.model_d
...
def image_encoder(image_feat,
  ...
  x = image_feat
  image_model_d = hparams.image_model_d or hparams.model_d
  image_d_ff = hparams.image_d_ff or hparams.d_ff
      ...
      with tf.variable_scope("layer_%d" % layer):
        with tf.variable_scope("self_attention"):
          y = vqa_layers.multihead_attention(
              ...
              None,
              None,
              hparams.attention_key_channels or image_model_d,
              hparams.attention_value_channels or image_model_d,
              image_model_d,
              hparams.num_heads,
...
def attn(image_feat,
  ...
  with tf.variable_scope(name, "attn", values=[image_feat, query]):
    total_key_depth = hparams.attention_key_channels or hparams.model_d
    total_value_depth = hparams.attention_value_channels or hparams.model_d
...
def vqa_self_attention_base():
  ...
  hparams.add_hparam("image_model_d", 0)  # default to model_d
  hparams.add_hparam("image_d_ff", 0)  # defaults to d_ff
