...
def add_edge_bias(x, d_ff):
  ...
  Args:
    ...
    d_ff: d_ff to determine padding.
  Returns:
    x_pad: Input tensor, shape (NHW(c+1))
  """
  x_shape = common_layers.shape_list(x)
  if d_ff[0] == 1 and d_ff[1] == 1:
    return x
  a = (d_ff[0] - 1) // 2  # vertical padding size
  b = (d_ff[1] - 1) // 2  # horizontal padding size
...
@add_arg_scope
def conv(name, x, output_channels, d_ff=None, stride=None,
  ...
  Args:
    ...
    d_ff: list of ints, if None [3, 3] and [2, 3, 3] are defaults for
  ...
  # set d_ff, stride and in_channels
  if is_2d:
    if d_ff is None:
      d_ff = [3, 3]
    if stride is None:
    ...
    x = time_pad(x, d_ff=d_ff, dilations=dilations)
    conv_filter = tf.nn.conv3d

  in_channels = common_layers.shape_list(x)[-1]
  filter_shape = d_ff + [in_channels, output_channels]
...
def conv_block(name, x, mid_channels, dilations=None, activation="relu",
  ...
  with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    ...
    # [output: 512 channels]
    x = conv("1_1", x, output_channels=mid_channels, d_ff=first_filter,
             dilations=dilations)
    x = tf.nn.relu(x)
    ...
    if activation == "relu":
      x = conv("1_2", x, output_channels=mid_channels,
               d_ff=second_filter, dilations=dilations)
      x = tf.nn.relu(x)
...
def temporal_latent_to_dist(name, x, hparams, output_channels=None):
  ...
  with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    ...
    h = conv("res_final", h, apply_actnorm=False, conv_init="zeros",
             output_channels=2*output_channels, d_ff=[1, 1])
